...
from database_service import (
    USERS_COLLECTION, COURSES_COLLECTION, TERMS_COLLECTION,
    COURSE_SECTIONS_COLLECTION, COURSE_OUTCOMES_COLLECTION
)
...
def get_users_by_role(role: str) -> List[Dict[str, Any]]:
    ...
    try:
        query = db.collection(USERS_COLLECTION).where(
            filter=firestore.FieldFilter("role", "==", role)
        ).where(
...
def create_course(course_data: Dict[str, Any]) -> Optional[str]:
    ...
    try:
        ...
        if not validate_course_number(course_data['course_number']):
            print(f"[DB Service Extended] Invalid course number format: {course_data['course_number']}")
            return None
        
        # Check if course already exists
        existing_course = get_course_by_number(course_data['course_number'])
        if existing_course:
            print(f"[DB Service Extended] Course already exists: {course_data['course_number']}")
            return None
...
def get_course_by_number(course_number: str) -> Optional[Dict[str, Any]]:
    ...
    try:
        query = db.collection(COURSES_COLLECTION).where(
            filter=firestore.FieldFilter("course_number", "==", course_number.upper().strip())
        ).limit(1)
...
def get_term_by_name(name: str) -> Optional[Dict[str, Any]]:
    ...
    try:
        query = db.collection(TERMS_COLLECTION).where(
            filter=firestore.FieldFilter("name", "==", name.strip())
        ).limit(1)
...
def get_sections_by_instructor(instructor_id: str) -> List[Dict[str, Any]]:
    ...
    """
    print(f"[DB Service Extended] get_sections_by_instructor called for: {instructor_id}")
    if not db:
        print("[DB Service Extended] Firestore client not available.")
        return []
...
__all__ = [
    'get_users_by_role', 'update_user_extended',
    'create_course', 'get_course_by_number', 'get_courses_by_department',
