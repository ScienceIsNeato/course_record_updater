...
class BaseAdapter:
    ...
    EXPECTED_FIELDS = {
        ...
        'num_students': (False, int, lambda n: n >= 0), # Now potentially required by grades
    ...
    def parse_and_validate(self, form_data: dict):
        ...
        validated_data = {}
        errors = []
        raw_input_data = {k: str(v).strip() for k, v in form_data.items()} # Store stripped strings
        ...
        any_grade_entered = any(raw_input_data.get(g_field) for g_field in self.GRADE_FIELDS)
        num_students_raw = raw_input_data.get('num_students')

        if any_grade_entered and not num_students_raw:
            errors.append("Number of students is required when entering grade distribution.")
            # Mark num_students as effectively required for subsequent checks if needed
            # This allows failing early if num_students is missing AND grades were entered
        ...
        for field, value_str in raw_input_data.items():
            if field in self.EXPECTED_FIELDS:
                ...
                # Skip empty optional fields (unless it's num_students and grades were entered)
                is_num_students_and_grades_entered = (field == 'num_students' and any_grade_entered)
                if not value_str and not is_required_field and not is_num_students_and_grades_entered:
                    continue 
                ...
                if expected_type:
                    try:
                        ...
                        else: # Handle case where field exists but type is None or unexpected
                             processed_value = value_str # Pass as string if no type specified
                    except (ValueError, TypeError):
                        errors.append(f"Invalid value for {field}: Cannot convert '{value_str}' to {expected_type.__name__}")
                        conversion_error = True
                else: # No expected type defined, treat as string
                     processed_value = value_str 
                
                if conversion_error: continue # Skip further validation on this field
                ...
                # Run specific field validator if conversion succeeded and validator exists
                if validator and processed_value is not None: # Check processed_value is not None
                    try:
                ...
                if field not in [e.split(':')[0].split(' ')[-1] for e in errors]: # Check if field caused error
                    # Only add if it has a value or is explicitly required (avoids adding None for empty optional fields)
        ...
        if any_grade_entered:
            ...
            if parsed_num_students is None and 'Number of students is required' not in "; ".join(errors):
                 # This implies num_students was provided but failed type conversion/validation
                ...
                if current_grade_sum != parsed_num_students:
                    errors.append(f"Sum of grades ({current_grade_sum}) does not match Number of Students ({parsed_num_students}).")
