...
class DatabaseService:
    ...
    def check_course_exists(self, course_data: Dict) -> Optional[str]:
        ...
        if not all(field in course_data for field in UNIQUE_FIELDS):
            print(f"[DB Service] check_course_exists: Missing unique fields ({UNIQUE_FIELDS}).")
            return None
    ...
    def _nursing_parse(self, document) -> List[Dict]:
        ...
        for paragraph in document.paragraphs:
            ...
            if text.startswith("Course:"):
                ...
                current_course["instructor_name"] = text.replace("Instructor:", "").strip()
    ...
    try:
        ...
        for course_data in validated_data_list:
            ...
            if isinstance(save_result, str) and save_result.startswith("DUPLICATE:"):
                upload_results["duplicate"].append(f"{course_identifier} - Already exists")
            elif save_result:
                upload_results["success"].append(f"{course_identifier} - Saved with ID: {save_result}")
            else:
...
if __name__ == '__main__':
    ...
    print(f"Firestore emulator: {'Yes' if os.environ.get('FIRESTORE_EMULATOR_HOST') else 'No'}")
    print("=" * 60)
