...
describe("Course Management - Edit Course Modal", () => {
  ...
  test("openEditCourseModal should populate form and show modal", async () => {
    ...
    await window.openEditCourseModal(
      "course-123",
      {
        course_number: "CS101",
        course_title: "Intro to CS",
    ...
    expect(document.getElementById("editCourseTitle").value).toBe(
      "Intro to CS",
    );
    expect(document.getElementById("editCourseDepartment").value).toBe(
      "Computer Science",
    );
    ...
    expect(
      document.getElementById("readOnlyProgramsDisplay").innerHTML,
    ).toContain("Programs");
  ...
  test("duplicateCourse should alert and return when API fails", async () => {
    ...
    await window.duplicateCourse("course-123", { course_number: "CS101" });
    expect(global.alert).toHaveBeenCalledWith(
      expect.stringContaining("Failed to duplicate course"),
    );
  });
  ...
  test("duplicateCourse should open edit modal when duplication succeeds (raw JSON string)", async () => {
    ...
    await window.duplicateCourse(
      "course-123",
      JSON.stringify({ course_number: "CS101" }),
    );
