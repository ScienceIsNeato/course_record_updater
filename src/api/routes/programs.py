...
"""Programs API routes for managing programs and course-program associations."""
...
def _get_current_user_safe() -> Dict[str, Any]:
    """Get current user, returning empty dict if None."""
    user = get_current_user()
    return user if user else {}


def _get_current_user_id_safe() -> str:
    """Get current user's ID, returning 'unknown' if not found."""
    user = _get_current_user_safe()
    return str(user.get("user_id") or "unknown")


def _get_current_institution_id_safe() -> str:
    """Get current institution ID, returning empty string if None."""
...
def _validate_program_for_removal(program_id: str) -> Tuple[Dict[str, Any], str]:
    ...
    return program, str(program.get("institution_id") or "")


def _remove_course_with_orphan_handling(
    course_id: str, program_id: str, institution_id: str, default_program_id: Optional[str]
) -> bool:
...
def _build_removal_response(success: bool, course_id: str, program: Dict[str, Any]) -> ResponseReturnValue:
    ...
    return jsonify({"success": False, "error": "Failed to remove course from program"}), 500
...
def _validate_bulk_manage_request() -> Optional[ResponseReturnValue]:
    ...
    if not action or action not in ["add", "remove"]:
        return jsonify({"success": False, "error": "Invalid or missing action. Use 'add' or 'remove'"}), 400
    if not course_ids or not isinstance(course_ids, list):
        return jsonify({"success": False, "error": "Missing or invalid course_ids array"}), 400
    return None
...
def _execute_bulk_add(course_ids: list, program_id: str) -> Tuple[Dict[str, Any], str]:
    ...
    return result, f"Bulk add operation completed: {result['success_count']} added"


def _execute_bulk_remove(course_ids: list, program_id: str) -> Tuple[Dict[str, Any], str]:
    ...
    if result.get("removed", 0) > 0 and default_program_id and institution_id:
        for course_id in course_ids:
            assign_course_to_default_program(course_id, institution_id)
    return result, f"Bulk remove operation completed: {result.get('removed', 0)} removed"
...
def delete_program_api(program_id: str) -> ResponseReturnValue:
    ...
    try:
        ...
        try:
            courses_for_program = get_courses_by_program(program_id)
        except Exception:
            courses_for_program = []
...
# ========================================
# COURSE-PROGRAM ASSOCIATION ENDPOINTS
# ========================================
