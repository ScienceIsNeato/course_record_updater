...
class TestImportBusinessLogic:
    ...
    def test_happy_path_first_import(self, mock_get_user, mock_create_course, mock_get_course):
        """
        ...
        try:
            ...
            assert result.success == True, f"Import should succeed, errors: {result.errors}"
            assert result.records_processed == 4, "Should process 4 records (2 courses × 2 sections)"
            assert result.records_created > 0, "Should create new records"
            assert result.conflicts_detected == 0, "Should not detect conflicts in empty database"
            assert len(result.errors) == 0, f"Should have no errors: {result.errors}"
            
            # Verify courses were created (2 unique courses)
            assert mock_create_course.call_count >= 2, "Should attempt to create courses"
    ...
    @patch('import_service.create_course')
    @patch('import_service.get_user_by_email')
    def test_identical_reimport_no_changes(self, mock_get_user, mock_create_course, mock_get_course):
        """
    ...
    @patch('import_service.create_course')
    @patch('import_service.get_user_by_email')
    def test_incremental_import_one_new_course(self, mock_get_user, mock_create_course, mock_get_course):
        """
        ...
        try:
            ...
            assert result.success == True, f"Import should succeed, errors: {result.errors}"
            assert result.records_processed == 6, "Should process 6 records (3 courses × 2 sections)"
            ...
            bio_course_calls = [call for call in mock_create_course.call_args_list 
    ...
    def test_data_deletion_protection(self, mock_get_course):
        ...
        existing_courses = {
            ...
            'CHEM-101': {'course_number': 'CHEM-101', 'course_title': 'General Chemistry'}
        }
        ...
        try:
            with patch('import_service.create_course') as mock_create:
                ...
                for call_args in mock_get_course.call_args_list:
                    ...
                    assert course_number in imported_course_numbers, \
        ...
        try:
            with patch('import_service.get_course_by_number') as mock_get_course, \
                ...
                expected_collections = [
                    'courses_v2', 'users', 'terms', 
                    'course_sections', 'course_outcomes', 'courses'
                ]
                
                collection_calls = [call[0][0] for call in mock_db.collection.call_args_list]
                for collection_name in expected_collections:
                    assert collection_name in collection_calls, \
                        f"Should attempt to delete {collection_name} collection"
                
                # Verify documents were deleted
                assert mock_doc.reference.delete.call_count > 0, \
