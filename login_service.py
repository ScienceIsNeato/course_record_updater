...
from password_service import PasswordService, AccountLockedError
...
class LoginService:
        ...
        try:
            ...
            if user.get('account_status') != 'active':
                if user.get('account_status') == 'pending':
                    raise LoginError("Account is pending activation. Please check your email for verification instructions.")
                elif user.get('account_status') == 'suspended':
                    raise LoginError("Account has been suspended. Please contact support.")
                elif user.get('account_status') == 'deactivated':
                    raise LoginError("Account has been deactivated. Please contact support.")
                else:
    ...
    def check_account_lockout_status(email: str) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.error("[Login Service] Error checking lockout status for %s: %s", email, str(e))
            return {
                'is_locked': False,
                'message': 'Unable to determine lockout status'
            }
    ...
    def send_account_locked_notification(email: str) -> bool:
        ...
        try:
            ...
            logger.warning("[Login Service] Account locked notification should be sent to: %s", email)
            
            return True
            
        except Exception as e:
            logger.error("[Login Service] Failed to send account locked notification to %s: %s", email, str(e))
            return False
    ...
    def unlock_account(email: str, admin_user_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info("[Login Service] Account unlocked for %s by admin %s", email, admin_user_id)
            ...
            logger.error("[Login Service] Failed to unlock account %s: %s", email, str(e))
