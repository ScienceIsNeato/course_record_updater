"""
Course Section Management API routes.
...
def _determine_section_filters(
    ...
    if not instructor_id and not term_id:
        if current_user.get("role") == UserRole.INSTRUCTOR.value:
            # Instructors see only their own sections
            return current_user.get("user_id"), None
...
def _filter_sections_by_permission(
    ...
    if current_user.get("role") == UserRole.INSTRUCTOR.value and not has_permission(
        "view_all_sections"
    ):
        # Ensure instructors only see their own sections
        return [
            s for s in sections if s.get("instructor_id") == current_user.get("user_id")
        ]
    return sections


def _verify_section_institution_access(
    section: Dict[str, Any], institution_id: str
) -> Optional[Tuple[Any, int]]:
    """Verify that the section belongs to the user's institution.

    Returns None if access is allowed, or an error response tuple if not.
    """
...
def get_section_by_id_endpoint(section_id: str) -> ResponseReturnValue:
    ...
    try:
        ...
        institution_id = _get_current_institution_id_safe()
        error_response = _verify_section_institution_access(section, institution_id)
        if error_response:
            return error_response
