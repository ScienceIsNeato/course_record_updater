...
class TestAPIRoutesHelperFunctions:
    """Test helper functions for API routes."""
    ...
    def test_resolve_program_override_success(self):
        """Test _resolve_program_override with valid program override."""
        from unittest.mock import patch

        from api_routes import _resolve_program_override

        current_user = {"role": "site_admin", "program_ids": ["prog1", "prog2"]}
        program_override = "prog1"

        with self.app.test_request_context("/?program_id=prog1"):
            with patch("api_routes.get_current_program_id") as mock_get_program:
                mock_get_program.return_value = "prog2"

                result = _resolve_program_override(current_user)

                assert result == program_override

    def test_resolve_program_override_no_override(self):
        """Test _resolve_program_override with no program override."""
        from unittest.mock import patch
        ...
        with self.app.test_request_context("/"):
            with patch("api_routes.get_current_program_id") as mock_get_program:
                mock_get_program.return_value = "prog1"
    ...
    def test_user_can_access_program_site_admin(self):
        ...
        result = _user_can_access_program(current_user, program_id)

        assert result is True

    def test_user_can_access_program_institution_admin(self):
        """Test _user_can_access_program for institution admin."""
        from api_routes import _user_can_access_program

        current_user = {"role": "institution_admin", "program_ids": ["prog1", "prog2"]}
        program_id = "prog1"
    ...
    def test_user_can_access_program_institution_admin_no_access(self):
        """Test _user_can_access_program for institution admin without access."""
        from api_routes import _user_can_access_program
    ...
    def test_get_global_courses_with_department_filter(self):
        """Test _get_global_courses with department filter."""
        from unittest.mock import patch
        ...
        institution_ids = ["inst1", "inst2"]
        department_filter = "MATH"
        mock_courses_1 = [
            {"id": "course1", "institution_id": "inst1", "department": "MATH"}
        ]
        mock_courses_2 = [
            {"id": "course2", "institution_id": "inst2", "department": "ENG"}
        ]

        with patch("api_routes.get_all_courses") as mock_get_courses:
            mock_get_courses.side_effect = [mock_courses_1, mock_courses_2]

            result = _get_global_courses(institution_ids, department_filter)

            assert mock_get_courses.call_count == 2
            assert result[0] == mock_courses_1  # Only MATH courses should be returned
            assert result[1] == "system-wide, department MATH"

    def test_get_global_courses_without_department_filter(self):
        """Test _get_global_courses without department filter."""
        from unittest.mock import patch
        ...
        institution_ids = ["inst1", "inst2"]
        mock_courses_1 = [{"id": "course1", "institution_id": "inst1"}]
        mock_courses_2 = [{"id": "course2", "institution_id": "inst2"}]

        with patch("api_routes.get_all_courses") as mock_get_courses:
            mock_get_courses.side_effect = [mock_courses_1, mock_courses_2]

            result = _get_global_courses(institution_ids, None)

            assert mock_get_courses.call_count == 2
            # Returns (courses, context_info)
            assert len(result[0]) == 2
            assert result[0][0]["id"] == "course1"
            assert result[0][1]["id"] == "course2"
            assert result[1] == "system-wide"

    def test_get_institution_courses_with_department_filter(self):
        """Test _get_institution_courses with department filter."""
        from unittest.mock import patch

        from api_routes import _get_institution_courses

        institution_id = "inst1"
        current_program_id = None  # No program ID to test institution-level courses
        department_filter = "MATH"
        mock_courses = [{"id": "course1"}, {"id": "course2"}]

        with patch("api_routes.get_courses_by_department") as mock_get_courses:
            mock_get_courses.return_value = mock_courses

            result = _get_institution_courses(
                institution_id, current_program_id, department_filter
            )

            mock_get_courses.assert_called_once_with(institution_id, department_filter)
            assert result[0] == mock_courses  # Returns (courses, context_info)
            assert result[1] == f"department {department_filter}"

    def test_get_institution_courses_without_department_filter(self):
        """Test _get_institution_courses without department filter."""
        from unittest.mock import patch
        ...
        with patch("api_routes.get_all_courses") as mock_get_courses:
            ...
            result = _get_institution_courses(institution_id, current_program_id, None)

            mock_get_courses.assert_called_once_with(institution_id)
            assert result[0] == mock_courses  # Returns (courses, context_info)

    def test_get_program_courses_with_department_filter(self):
        """Test _get_program_courses with department filter."""
        from unittest.mock import patch
        ...
        department_filter = "MATH"
        mock_courses = [
            {"id": "course1", "department": "MATH"},
            {"id": "course2", "department": "ENG"},
        ]

        with patch("api_routes.get_courses_by_program") as mock_get_courses:
            mock_get_courses.return_value = mock_courses

            result = _get_program_courses(program_id, department_filter)

            mock_get_courses.assert_called_once_with(program_id)
            # Should only return MATH courses after filtering
            assert len(result[0]) == 1
            assert result[0][0]["department"] == "MATH"
            assert result[1] == f"program {program_id}"

    def test_get_program_courses_without_department_filter(self):
        """Test _get_program_courses without department filter."""
        from unittest.mock import patch
        ...
        with patch("api_routes.get_courses_by_program") as mock_get_courses:
            ...
            result = _get_program_courses(program_id, None)
    ...
    def test_validate_program_for_removal_success(self):
        ...
        with patch("api_routes.get_program_by_id") as mock_get_program:
            ...
            result = _validate_program_for_removal(program_id)

            assert result == (mock_program, "inst1")

    def test_validate_program_for_removal_not_found(self):
        """Test _validate_program_for_removal with non-existent program."""
        from unittest.mock import patch
        ...
        with patch("api_routes.get_program_by_id") as mock_get_program:
            ...
            with pytest.raises(ValueError, match="Program not found"):
                _validate_program_for_removal(program_id)

    def test_get_default_program_id_success(self):
        """Test _get_default_program_id with valid institution."""
        from unittest.mock import patch

        from api_routes import _get_default_program_id

        institution_id = "inst1"
        mock_program = {"id": "prog1", "name": "Default Program", "is_default": True}

        with patch("api_routes.get_programs_by_institution") as mock_get_programs:
            mock_get_programs.return_value = [mock_program]

            result = _get_default_program_id(institution_id)

            assert result == "prog1"

    def test_get_default_program_id_no_programs(self):
        """Test _get_default_program_id with no programs."""
        from unittest.mock import patch
    ...
    def test_build_removal_response_success(self):
        """Test _build_removal_response for successful removal."""
        from api_routes import _build_removal_response

        success = True
        course_id = "course1"
        program = {"id": "prog1", "name": "Test Program"}

        with self.app.app_context():
            result = _build_removal_response(success, course_id, program)

            assert result.json["success"] is True
            assert (
                result.json["message"]
                == f"Course {course_id} removed from program {program['name']}"
            )

    def test_build_removal_response_failure(self):
        """Test _build_removal_response for failed removal."""
        from api_routes import _build_removal_response
        ...
        with self.app.app_context():
            ...
            # Result is a tuple (response, status_code) for failures
            assert result[0].json["success"] is False
            assert result[0].json["error"] == "Failed to remove course from program"
            assert result[1] == 500

    def test_validate_bulk_manage_request_valid(self):
        """Test _validate_bulk_manage_request with valid data."""
        from api_routes import _validate_bulk_manage_request

        mock_data = {"action": "add", "course_ids": ["course1", "course2"]}
    ...
    # TODO: Fix Flask request context mocking for this test
    ...
    def test_validate_bulk_manage_request_invalid_action(self):
        ...
        mock_data = {"action": "invalid", "course_ids": ["course1", "course2"]}
        ...
        with self.app.test_request_context(json=mock_data):
            ...
            assert result[0].json["success"] is False
            assert "Invalid or missing action" in result[0].json["error"]

    def test_validate_bulk_manage_request_no_course_ids(self):
        """Test _validate_bulk_manage_request with no course_ids."""
        from api_routes import _validate_bulk_manage_request

        mock_data = {"action": "add", "course_ids": []}
        ...
        with self.app.test_request_context(json=mock_data):
            ...
            assert result is not None
            assert result[0].json["success"] is False
            assert "Missing or invalid course_ids" in result[0].json["error"]

    def test_execute_bulk_add_success(self):
        """Test _execute_bulk_add with successful operation."""
        from unittest.mock import patch
        ...
        program_id = "prog1"
        mock_result = {"success_count": 2, "failed_count": 0}
        ...
        with patch("api_routes.bulk_add_courses_to_program") as mock_bulk_add:
            ...
            mock_bulk_add.assert_called_once_with(course_ids, program_id)
            assert result == mock_result
            assert "Bulk add operation completed: 2 added" in message

    def test_execute_bulk_remove_success(self):
        """Test _execute_bulk_remove with successful operation."""
        from unittest.mock import patch
        ...
        with patch("api_routes.bulk_remove_courses_from_program") as mock_bulk_remove:
            with patch("api_routes._get_default_program_id") as mock_get_default:
                with patch("api_routes.get_current_institution_id") as mock_get_inst:
                    with patch(
                        "api_routes.assign_course_to_default_program"
                    ) as mock_assign:
                        ...
                        mock_bulk_remove.assert_called_once_with(course_ids, program_id)
                        assert result == mock_result
                        assert "Bulk remove operation completed: 2 removed" in message

    # TODO: Fix Flask file upload context mocking for Excel import tests
    # These tests need proper Flask request context with file uploads
    def _test_validate_excel_import_request_valid(self):
        """Test _validate_excel_import_request with valid request."""
        from api_routes import _validate_excel_import_request

        mock_data = {
            "file": "test.xlsx",
            "adapter_id": "cei_excel",
            "import_data_type": "courses",
        }

        with self.app.test_request_context(json=mock_data):
            result = _validate_excel_import_request()

            assert result is None

    def _test_validate_excel_import_request_no_data(self):
        """Test _validate_excel_import_request with no data."""
        from api_routes import _validate_excel_import_request
        ...
        with self.app.test_request_context(json=None):
            ...
            assert result is not None
            assert result[0].json["success"] is False
            assert "No data provided" in result[0].json["error"]

    def _test_validate_excel_import_request_missing_file(self):
        """Test _validate_excel_import_request with missing file."""
        from api_routes import _validate_excel_import_request

        mock_data = {"adapter_id": "cei_excel", "import_data_type": "courses"}
        ...
        with self.app.test_request_context(json=mock_data):
            ...
            assert result is not None
            assert result[0].json["success"] is False
            assert "Missing required field: file" in result[0].json["error"]

    def _test_check_excel_import_permissions_site_admin(self):
        ...
        with patch("api_routes.get_current_user") as mock_get_user:
            mock_get_user.return_value = {"role": "site_admin"}

            result = _check_excel_import_permissions(adapter_id, import_data_type)

            assert result is None

    def _test_check_excel_import_permissions_institution_admin(self):
        """Test _check_excel_import_permissions for institution admin."""
        from unittest.mock import patch
        ...
        with patch("api_routes.get_current_user") as mock_get_user:
            mock_get_user.return_value = {"role": "institution_admin"}

            result = _check_excel_import_permissions(adapter_id, import_data_type)

            assert result is None

    def _test_check_excel_import_permissions_insufficient_permissions(self):
        """Test _check_excel_import_permissions with insufficient permissions."""
        from unittest.mock import patch
        ...
        with patch("api_routes.get_current_user") as mock_get_user:
            mock_get_user.return_value = {"role": "student"}
    ...
    def _test_determine_target_institution_site_admin(self):
        """Test _determine_target_institution for site admin."""
        from api_routes import _determine_target_institution

        user_role = "site_admin"
        user_institution_id = "inst1"
        adapter_id = "cei_excel"
        ...
        assert result == user_institution_id
    ...
    def test_validate_import_permissions_site_admin(self):
        """Test _validate_import_permissions for site admin."""
        from api_routes import _validate_import_permissions

        user_role = "site_admin"
        import_data_type = "courses"

        result = _validate_import_permissions(user_role, import_data_type)

        assert result is None

    def test_validate_import_permissions_institution_admin(self):
        """Test _validate_import_permissions for institution admin."""
        from api_routes import _validate_import_permissions
    ...
    def _test_validate_import_permissions_insufficient_permissions(self):
        """Test _validate_import_permissions with insufficient permissions."""
        from api_routes import _validate_import_permissions
    ...
    def _test_process_excel_import_success(self):
        """Test _process_excel_import with successful import."""
        from unittest.mock import MagicMock, patch

        from api_routes import _process_excel_import

        mock_file = MagicMock()
        mock_file.filename = "test.xlsx"
        current_user = {"role": "site_admin", "institution_id": "inst1"}
        institution_id = "inst1"
        ...
        with patch("import_service.ImportService") as mock_import_service:
            mock_import_service.return_value.import_excel_file.return_value = (
                mock_result
            )

            result = _process_excel_import(
                mock_file, current_user, institution_id, import_params
            )

            assert result == mock_result
            mock_import_service.assert_called_once()

    def _test_process_excel_import_failure(self):
        """Test _process_excel_import with import failure."""
        from unittest.mock import MagicMock, patch
        ...
        with patch("import_service.ImportService") as mock_import_service:
            mock_import_service.return_value.import_excel_file.side_effect = Exception(
                "Import failed"
            )
            ...
            assert result is not None
            assert result[0].json["success"] is False
            assert "Import failed" in result[0].json["error"]
