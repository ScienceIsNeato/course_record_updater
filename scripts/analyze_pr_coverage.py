...
def _get_raw_diff_output(base_branch):
    ...
    return subprocess.run(  # nosec
        ["git", "diff", base_branch, "HEAD"], capture_output=True, text=True, check=True
    )
        ...
        for package in root.findall(".//package"):
            for cls in package.findall("classes/class"):
                ...
                for line in cls.findall("lines/line"):
                    ...
                    if is_branch:
                        condition_coverage = line.get(
                            "condition-coverage", "100% (0/0)"
                        )
                        # Parse "50% (1/2)" format
                        if "(" in condition_coverage:
                            try:
                                # Extract "1/2" from "50% (1/2)"
                                fraction = condition_coverage.split("(")[1].split(")")[
                                    0
                                ]
...
def print_report(pr_coverage_gaps: Dict[str, Set[int]], output_file: str = None):
    ...
    if not pr_coverage_gaps:
        ...
        message = (
            metadata + "✅ All NEWLY ADDED lines in this PR are covered by tests!\n"
        )
        print(message)
        if output_file:
    ...
    sorted_files = sorted(
        production_gaps.items(), key=lambda x: len(x[1]), reverse=True
    )
    ...
    for filepath, line_numbers in sorted_files:
        ...
        lines.append(
            f"   🔴 {len(line_numbers)} uncovered lines: {format_line_ranges(line_numbers)}"
        )
        lines.append("")
...
def main():
    ...
    python_uncovered, python_covered_files = get_uncovered_lines_from_xml(
        args.coverage_file
    )
    python_uncovered_count = sum(len(lines) for lines in python_uncovered.values())
    print(
        f"   📊 Python: {python_uncovered_count} total uncovered lines across {len(python_uncovered)} files"
    )
    ...
    pr_coverage_gaps = cross_reference_coverage(
        added_lines, uncovered_lines, all_covered_files
    )
