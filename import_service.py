...
from models import (
    User, Course, Term, CourseSection, CourseOutcome,
...
from database_service_extended import (
    create_course, create_term, create_course_section,
    get_course_by_number, get_term_by_name
)
...
class ImportService:
    ...
    def detect_course_conflict(self, import_course: Dict[str, Any]) -> List[ConflictRecord]:
        """
    ...
    def resolve_conflict(self, conflict: ConflictRecord, strategy: ConflictStrategy) -> str:
        """
    ...
    def process_course_import(
        ...
        if conflicts:
            ...
            if course_number not in self._processed_courses:
                ...
                self._log(f"Resolving {len(conflicts)} conflicts for course: {course_number}", "summary")
            ...
            if not dry_run and strategy == ConflictStrategy.USE_THEIRS:
                ...
                if field_conflicts:
                    ...
                    if existing_course:
                        ...
                        self.stats['errors'].append(f"Course not found for update: {course_number}")
                        return False, conflicts
                else:
                    # Course exists with identical data - no action needed
                    self.stats['records_skipped'] += 1
                    course_number = course_data.get('course_number')
                    self._log(f"Course already exists with identical data: {course_number}", "debug")
            ...
            if not dry_run:
                ...
                if course_id:
                    ...
                    print(f"[Import] Created course: {course_data.get('course_number')}")
                else:
                    self.stats['errors'].append(f"Failed to create course: {course_data.get('course_number')}")
                    return False, conflicts
            else:
                print(f"[Import] DRY RUN: Would create course: {course_data.get('course_number')}")
    ...
    def process_user_import(
        ...
        if conflicts:
            ...
            if not dry_run and strategy == ConflictStrategy.USE_THEIRS:
                ...
                # Skip the import, keep existing
                self.stats['records_skipped'] += 1
                print(f"[Import] Skipped user (keeping existing): {user_data.get('email')}")
            ...
            if not dry_run:
                ...
                if user_id:
                    ...
                    self.stats['errors'].append(f"Failed to create user: {user_data.get('email')}")
                    return False, conflicts
            else:
                email = user_data.get('email')
                if email not in self._processed_users:
    ...
    def import_excel_file(
        ...
        if delete_existing_db:
            print(f"[Import] ⚠️  DELETE MODE: Will clear existing database before import")
        ...
        try:
            ...
            try:
                ...
                self._log(f"Loaded Excel file: {len(df)} rows, {len(df.columns)} columns", "summary")
            ...
            for index, row in df.iterrows():
                ...
                if index % progress_interval == 0 or index == total_rows - 1:
                    ...
                    self._log(f"Processing row {index + 1}/{total_rows} ({progress}%)", "summary")
                    ...
                    for entity_type, entity_data in entities.items():
                        if entity_type == 'course' and entity_data:
                            ...
                            if not dry_run:
                                ...
                                if term_id:
                                    self.stats['records_created'] += 1
                                    print(f"[Import] Created term: {entity_data.get('name')}")
                            else:
                                print(f"[Import] DRY RUN: Would create term: {entity_data.get('name')}")
                            ...
                            if not dry_run:
                                ...
                                if section_id:
                                    self.stats['records_created'] += 1
                                    print(f"[Import] Created section: {entity_data.get('course_id')}")
                            else:
                                print(f"[Import] DRY RUN: Would create section: {entity_data.get('course_id')}")
    ...
    def _parse_cei_excel_row(self, row: pd.Series) -> Dict[str, Optional[Dict[str, Any]]]:
        """
        ...
        try:
            ...
            if 'course' in row and pd.notna(row['course']):
                ...
                if validate_course_number(course_number):
                    course_data = {
                        ...
                        'department': self._extract_department_from_course(course_number),
            ...
            if course_data and user_data and term_data:
                section_data = {
                    'course_id': course_data.get('course_number'),  # Will be resolved later
                    ...
                    'enrollment': int(row.get('Enrolled Students', 0)) if pd.notna(row.get('Enrolled Students')) else None,
    ...
    def _create_import_result(self, start_time: datetime, dry_run: bool) -> ImportResult:
...
def create_import_report(result: ImportResult) -> str:
    ...
    if result.conflicts:
        ...
        for i, conflict in enumerate(result.conflicts, 1):
            report.append(f"  {i}. {conflict.entity_type} '{conflict.entity_key}' - {conflict.field_name}")
