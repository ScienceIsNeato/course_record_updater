...
if database_client is None:
    app.logger.error("database_service failed to initialize Firestore client. Database operations will fail.")
else:
...
if __name__ == '__main__':
    ...
    port = int(os.environ.get('PORT', 
                             os.environ.get('COURSE_RECORD_UPDATER_PORT', 3001)))
    # Debug mode should be controlled by FLASK_DEBUG environment variable
    use_debug = os.environ.get('FLASK_DEBUG', 'false').lower() == 'true'

    print(f"INFO: Starting Course Record Updater on port {port} with debug mode: {use_debug}")
    print(f"INFO: Access the application at http://localhost:{port}")
    print(f"INFO: To use a different port, set PORT environment variable: PORT=3002 python app.py")
