...
Provides endpoints for creating, managing, and accepting user invitations.
"""
...
@invitations_bp.route("/auth/invite", methods=["POST"])
...
@invitations_bp.route("/auth/accept-invitation", methods=["POST"])
...
@invitations_bp.route("/auth/invitation-status/<invitation_token>", methods=["GET"])
...
@invitations_bp.route("/auth/resend-invitation/<invitation_id>", methods=["POST"])
@permission_required("manage_institution_users")
...
@invitations_bp.route("/auth/invitations", methods=["GET"])
@permission_required("manage_institution_users")
...
@invitations_bp.route("/auth/cancel-invitation/<invitation_id>", methods=["DELETE"])
@permission_required("manage_institution_users")
def cancel_invitation_api(invitation_id: str) -> ResponseReturnValue:
    ...
    try:
        ...
        if success:
            return (
                jsonify({"success": True, "message": "Invitation cancelled successfully"}),
                200,
            )
        else:
            return (
