...
Provides endpoints for managing academic terms within an institution.
"""
...
def list_terms() -> ResponseReturnValue:
    """
    Get list of terms for the current institution.

    Use ?all=true to get all terms, otherwise returns only active terms.

    Query Parameters:
        - all: Include all terms (active and inactive). Default: false

    Returns:
        200: List of terms
        400: Institution context required
        500: Server error
    """
    try:
        try:
...
def create_term_api() -> ResponseReturnValue:
    ...
    Request Body:
        {
            "name": str (required) - Term name (e.g., "2024 Fall")
            "start_date": str (required) - Start date (YYYY-MM-DD)
            "end_date": str (required) - End date (YYYY-MM-DD)
            "assessment_due_date": str (optional) - Assessment due date (YYYY-MM-DD)
        }

    Returns:
        201: Term created successfully
        400: Missing required fields or institution context
        500: Server error
    """
    try:
...
def update_term_endpoint(term_id: str) -> ResponseReturnValue:
    ...
    Returns:
        ...
        400: Invalid data or date format
        404: Term not found
        500: Server error
    """
    try:
