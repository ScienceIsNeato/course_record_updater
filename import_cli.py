...
from import_service import import_excel, create_import_report, ConflictStrategy, ImportResult
...
Options:
  ...
  --verbose        Show detailed progress information
    ...
    strategy_group.add_argument(
        '--use-mine',
        action='store_true',
        help='Keep existing data, skip conflicts'
    )
    strategy_group.add_argument(
        '--use-theirs',
        action='store_true',
    ...
    strategy_group.add_argument(
        '--merge',
        action='store_true',
        help='Merge conflicting data intelligently'
    )
    strategy_group.add_argument(
        '--manual-review',
        action='store_true',
        help='Flag conflicts for manual review'
    )
    ...
    if result.conflicts_detected > 0:
        ...
        if verbose and result.conflicts:
            ...
            for i, conflict in enumerate(result.conflicts[:10], 1):  # Show first 10
                print(f"   {i}. {conflict.entity_type} '{conflict.entity_key}' - {conflict.field_name}")
