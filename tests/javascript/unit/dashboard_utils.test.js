...
describe("dashboard_utils.js", () => {
  ...
  describe("deriveTimelineStatus", () => {
    test("returns SCHEDULED when before start date", () => {
      ...
      expect(dashboardUtils.deriveTimelineStatus(start, end, ref)).toBe(
        "SCHEDULED",
      );
    });
    ...
    test("returns UNKNOWN for missing dates", () => {
      expect(dashboardUtils.deriveTimelineStatus(null, "2024-06-01")).toBe(
        "UNKNOWN",
      );
      expect(dashboardUtils.deriveTimelineStatus("2024-01-01", null)).toBe(
        "UNKNOWN",
      );
  ...
  describe("resolveTimelineStatus", () => {
    ...
    test("derives status from start/end dates when no direct status", () => {
      ...
      expect(
        dashboardUtils.resolveTimelineStatus(record, { referenceDate: ref }),
      ).toBe("PASSED");
    });
