...
class CourseSection(DataModel):
    ...
    def create_schema(
        ...
        if status not in SECTION_STATUSES:
            raise ValueError(f"Invalid status: {status}. Must be one of {SECTION_STATUSES}")
        ...
        return {
            ...
            'assigned_date': CourseSection.current_timestamp() if instructor_id else None,
            'completed_date': None,
...
class CourseOutcome(DataModel):
    ...
    def create_schema(
        ...
        return {
            ...
            'assessment_method': assessment_method.strip() if assessment_method else None,
            'assessment_data': {
    ...
    def update_assessment_data(
        ...
        if assessment_status not in ASSESSMENT_STATUSES:
            raise ValueError(f"Invalid assessment_status: {assessment_status}. Must be one of {ASSESSMENT_STATUSES}")
...
class LegacyCourse(DataModel):
    ...
    @staticmethod
    def to_relational_entities(course_data: Dict[str, Any]) -> Dict[str, Dict[str, Any]]:
