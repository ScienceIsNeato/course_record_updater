...
def compare_zip_files(export1_path: str, export2_path: str) -> dict:
    ...
    with zipfile.ZipFile(export1_path, "r") as zf1, zipfile.ZipFile(export2_path, "r") as zf2:
        # Compare file lists
        ...
        if counts1 != counts2:
            differences.append(f"Entity counts differ:\n  Export1: {counts1}\n  Export2: {counts2}")
        else:
        ...
        for filename in files1 & files2:
            ...
            if content1 != content2:
                ...
                if len(lines1) != len(lines2):
                    differences.append(f"{filename}: Row count differs ({len(lines1)} vs {len(lines2)})")
                else:
                    # CSV content might have timestamp differences - that's expected
                    logger.info(f"⚠️  {filename}: Content differs (likely timestamp variations - EXPECTED)")
...
def main():
    ...
    logger.info("")
    logger.info("🔍 You can extract and inspect both ZIP files to verify data integrity")
