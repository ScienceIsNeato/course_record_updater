...
} = require("../../../static/bulk_reminders.js");
...
describe("BulkReminderManager", () => {
  ...
  describe("Selection Management", () => {
    ...
    test("should update selection count when checkboxes change", () => {
      ...
      expect(
        document.getElementById("selectedInstructorCount").textContent,
      ).toBe("2");
      expect(document.getElementById("sendRemindersButton").disabled).toBe(
        false,
      );
    });
  ...
  describe("Sending Reminders", () => {
    ...
    test("should switch to progress view and start polling", async () => {
      ...
      expect(document.getElementById("reminderStep1").style.display).toBe(
        "none",
      );
      expect(document.getElementById("reminderStep2").style.display).toBe(
        "block",
      );
    ...
    test("should handle send error gracefully", async () => {
      ...
      expect(consoleErrorSpy).toHaveBeenCalled();
      expect(document.getElementById("closeProgressButton").disabled).toBe(
        false,
      );
    });
  ...
  describe("Progress Tracking", () => {
    ...
    test("should check job status via API", async () => {
      ...
      expect(mockFetch).toHaveBeenCalledWith(
        "/api/bulk-email/job-status/job-123",
      );
    });
    ...
    test("should update progress bar and counts", () => {
      ...
      expect(document.getElementById("reminderProgressText").textContent).toBe(
        "75%",
      );
      expect(document.getElementById("reminderSentCount").textContent).toBe(
        "15",
      );
      expect(document.getElementById("reminderFailedCount").textContent).toBe(
        "2",
      );
      expect(document.getElementById("reminderPendingCount").textContent).toBe(
        "3",
      );
    });
  ...
  describe("Job Completion", () => {
    ...
    test("should update progress bar styling on completion", () => {
      ...
      expect(progressBar.classList.contains("progress-bar-animated")).toBe(
        false,
      );
  ...
  describe("Modal Reset", () => {
    ...
    test("should reset progress view", () => {
      ...
      expect(document.getElementById("reminderComplete").style.display).toBe(
        "none",
      );
    });
  ...
  describe("Global Functions", () => {
    ...
    test("openBulkReminderModal should handle uninitialized manager", () => {
      ...
      expect(consoleErrorSpy).toHaveBeenCalledWith(
        "[BulkReminders] Manager not initialized",
      );
    });
  });
});
