...
def get_package_mappings(filepath):
    ...
    if '/src/database/' in path_str:
        return {**IMPORT_MAPPINGS.get('src/database', {}), **IMPORT_MAPPINGS.get('src', {})}
    elif '/src/models/' in path_str:
        return {**IMPORT_MAPPINGS.get('src/models', {}), **IMPORT_MAPPINGS.get('src', {})}
    elif '/src/services/' in path_str:
        return {**IMPORT_MAPPINGS.get('src/services', {}), **IMPORT_MAPPINGS.get('src', {})}
            ...
            for old_pattern, new_import in mappings.items():
                if re.match(old_pattern, line.strip()):
                    ...
                    if updated_line != line:
                        changes_made.append(f"  Line {line_num}: {line.strip()} -> {updated_line.strip()}")
                    break
