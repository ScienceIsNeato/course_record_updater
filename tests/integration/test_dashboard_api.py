...
class TestDashboardAPI:
    ...
    def test_main_dashboard_endpoints(self, base_url: str):
        ...
        for endpoint in endpoints:
            try:
                ...
                ), f"Endpoint {endpoint} returned 404 - endpoint missing!"

                # If we get 400/401/403, that's better than 404 - means endpoint exists but needs auth
                if response.status_code in [400, 401, 403]:
                    print(
                        f"Endpoint {endpoint} requires auth (status {response.status_code}) - but exists!"
                    )
                    continue
                ...
                if response.status_code == 200:
                    ...
                    assert isinstance(
                        data, dict
                    ), f"Endpoint {endpoint} didn't return JSON object"
                    assert (
                        "success" in data
    ...
    def test_debug_course_data_structure(self, base_url: str):
        ...
        if data["total_count"] > 0:
            ...
            assert (
                len(sample_courses) > 0
            ), "No sample courses returned despite total_count > 0"
    ...
    def test_debug_instructor_data_structure(self, base_url: str):
        ...
        if data["total_count"] > 0:
            ...
            assert (
                len(sample_instructors) > 0
            ), "No sample instructors returned despite total_count > 0"
            ...
            for field in required_fields:
                assert (
                    field in first_instructor
                ), f"Instructor missing required field: {field}"
    ...
    def test_debug_section_data_structure(self, base_url: str):
        ...
        if data["total_count"] > 0:
            ...
            assert (
                len(sample_sections) > 0
            ), "No sample sections returned despite total_count > 0"
    ...
    def test_debug_term_data_structure(self, base_url: str):
        ...
        if data["total_count"] > 0:
            ...
            assert (
                len(sample_terms) > 0
            ), "No sample terms returned despite total_count > 0"
    ...
    def test_dashboard_consistency(self, base_url: str):
        ...
        for entity_type in main_endpoints:
            ...
            # Counts should match
            assert (
                main_count == debug_count
            ), f"Count mismatch for {entity_type}: main={main_count}, debug={debug_count}"
...
class TestDashboardFrontend:
    ...
    def test_debug_section_present(self, base_url: str, driver):
        ...
        assert (
            len(debug_elements) >= 4
        ), f"Expected 4 debug elements, found {len(debug_elements)}"
