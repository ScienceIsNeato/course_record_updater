...
class EmailManager:
    ...
    def send_all(
        ...
        while True:
            ...
            with self.lock:
                if success:
                    ...
                    stats["sent"] += 1
                    logger.info(
                        f"[EmailManager] Successfully sent email to {job.to_email}"
                    )
                else:
        ...
        with self.lock:
            stats["pending"] = sum(
                1 for j in self.queue if j.status == EmailStatus.PENDING
            )
    ...
    def _send_with_retry(
        ...
        for attempt in range(self.max_retries):
            ...
            try:
                ...
                success = send_func(
                    job.to_email, job.subject, job.html_body, job.text_body
                )
    ...
    def get_status(self) -> Dict[str, int]:
        ...
        with self.lock:
            return {
                ...
                "sending": sum(
                    1 for j in self.queue if j.status == EmailStatus.SENDING
                ),
