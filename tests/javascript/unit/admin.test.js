...
describe('admin module', () => {
  ...
  it('applies default invite workflow when opening the modal', () => {
    ...
    expect(document.getElementById('sectionAssignmentGroup').style.display).toBe('block');
    expect(document.getElementById('inviteSectionId').value).toBe('section-123');
    expect(document.getElementById('inviteRole').value).toBe('program_admin');
    const programOption = Array.from(document.getElementById('invitePrograms').options).find(
      (option) => option.value === 'prog-1',
    );
  ...
  it('includes first and last names when provided', async () => {
    ...
    const showErrorSpy = jest.spyOn(admin, 'showError').mockImplementation(() => {});
    const showLoadingSpy = jest.spyOn(admin, 'showLoading').mockImplementation(() => {});
    const showEmptySpy = jest.spyOn(admin, 'showEmpty').mockImplementation(() => {});
  ...
  describe('editUser', () => {
    ...
    it('populates form with user data when user found', async () => {
      ...
      expect(document.getElementById('editEmail').value).toBe('john@example.com');
  ...
  describe('openInviteModal', () => {
    ...
    it('opens modal with default workflow when no options provided', () => {
      admin.openInviteModal();
      expect(global.bootstrap.Modal).toHaveBeenCalled();
      expect(global.bootstrap.Modal.mock.results[0].value.show).toHaveBeenCalled();
    });
    ...
    it('warns when modal element not found', () => {
      ...
      expect(consoleSpy).toHaveBeenCalledWith('Invite modal is not present in the current DOM.');
    ...
    it('passes correct context to workflow functions', () => {
      ...
      admin.registerInviteModalWorkflow('test', {
        reset: jest.fn(),
        setup: (context) => { capturedContext = context; }
      });
    ...
    it('handles workflow without reset function', () => {
      ...
      expect(() => admin.openInviteModal({ workflow: 'no-reset' })).not.toThrow();
    });
    ...
    it('handles workflow without setup function', () => {
      ...
      expect(() => admin.openInviteModal({ workflow: 'no-setup' })).not.toThrow();
    });
  ...
  describe('default workflow', () => {
    ...
    it('applies programId selection when provided', (done) => {
      ...
      admin.openInviteModal({ programId: 'prog-1', prefillRole: 'program_admin' });
      
      // Program selection happens immediately if options exist
      setTimeout(() => {
        const selected = Array.from(programsSelect.selectedOptions).map(o => o.value);
  ...
  describe('setProgramSelectionVisibility', () => {
    ...
    it('handles missing DOM elements gracefully', () => {
      setBody('<div></div>');
      expect(() => admin.setProgramSelectionVisibility('program_admin')).not.toThrow();
    });
  });
