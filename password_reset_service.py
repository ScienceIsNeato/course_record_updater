        ...
        try:
            ...
            if user.get('account_status') != 'active':
                if user.get('account_status') == 'pending':
                    raise PasswordResetError("Account is pending activation. Please verify your email first.")
                elif user.get('account_status') == 'suspended':
                    raise PasswordResetError("Account is suspended. Please contact support.")
                elif user.get('account_status') == 'deactivated':
                    raise PasswordResetError("Account is deactivated. Please contact support.")
                else:
            ...
            if not success:
                raise PasswordResetError("Failed to generate reset token. Please try again.")
            ...
            logger.error("Email service error during password reset for %s: %s", email, str(e))
            if "protected domain" in str(e).lower():
                # In non-production, inform user about email protection
                raise PasswordResetError("Email sending is restricted in development environment.")
            else:
                raise PasswordResetError("Failed to send reset email. Please try again.")
        except Exception as e:
            logger.error("Password reset request failed for %s: %s", email, str(e))
            if isinstance(e, PasswordResetError):
                raise
            else:
                raise PasswordResetError("Password reset request failed. Please try again.") from e
    ...
    def reset_password(reset_token: str, new_password: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if not token_data or not PasswordService.is_reset_token_valid(token_data):
                raise PasswordResetError("Reset token has expired or been used. Please request a new one.")
            ...
            try:
                ...
                logger.warning("Failed to send password reset confirmation email to %s: %s", 
                             user['email'], str(e))
            ...
            logger.error("Password reset failed for token %s: %s", reset_token[:8] + "...", str(e))
            if isinstance(e, PasswordResetError):
                raise
            else:
                raise PasswordResetError("Password reset failed. Please try again.") from e
    ...
    def _check_reset_rate_limit(email: str) -> None:
        ...
        try:
            ...
            if "rate limit" in str(e).lower():
                raise PasswordResetError("Too many password reset requests. Please wait before trying again.") from e
            else:
