...
class RoundtripValidator:
    ...
    def validate_roundtrip(
        ...
        try:
            ...
            export_result = self.export_service.export_data(
                config=export_config, output_path=export_file
            )
            ...
            comparison_result = self._compare_files(
                input_file, export_file, adapter_name
            )
    ...
    def _clean_database(self):
        ...
        LOGGER.warning(
            "Database cleanup not implemented - using delete_existing_db flag instead"
        )

    def _compare_files(
        self, original_file: Path, exported_file: Path, adapter_name: str
    ) -> ValidationResult:
        """
        ...
        try:
            ...
            return ValidationResult(
                success=False, errors=[f"File comparison failed: {str(e)}"]
            )

    def _compare_cei_files(
        self, original_file: Path, exported_file: Path
    ) -> ValidationResult:
        """
        ...
        try:
            ...
            for field in key_fields:
                if field in original_df.columns and field in exported_df.columns:
                    ...
                    if missing_values:
                        differences.append(f"Missing {field} values: {missing_values}")

                    if extra_values:
                        warnings.append(f"Extra {field} values: {extra_values}")
            ...
            if success:
                LOGGER.info(
                    "File comparison successful - no critical differences found"
                )
            else:
                LOGGER.warning(f"File comparison found {len(differences)} differences")

            return ValidationResult(
                success=success, differences=differences, warnings=warnings
            )

        except Exception as e:
            return ValidationResult(
                success=False, errors=[f"CEI file comparison failed: {str(e)}"]
            )
...
def main():
    ...
    parser.add_argument(
        "input_file", type=Path, help="Excel file to test roundtrip validation on"
    )
    ...
    result = validator.validate_roundtrip(
        input_file=args.input_file, adapter_name=args.adapter, temp_dir=args.output_dir
    )
