"""
Offerings API routes.

Provides endpoints for managing course offerings (courses scheduled in specific terms).
"""
...
def _enrich_offerings_with_programs(
    ...
    """Add program_names and program_ids to each offering."""
...
def _build_term_lookup(terms: List[Dict[str, Any]]) -> Dict[str, Dict[str, Any]]:
    """Build a lookup dictionary from term_id to term data."""
...
def _filter_offerings_by_term(
    ...
    """Filter offerings by term_id if specified."""
    if not term_id:
        return offerings
...
def _filter_offerings_by_course(
    ...
    """Filter offerings by course_id if specified."""
    if not course_id:
        return offerings
...
def _populate_offering_metadata(
    ...
    """Populate offering with section stats and course/term metadata."""
...
def _build_offering_context(
    ...
    """Build context data needed to enrich offerings."""
...
def _get_filtered_offerings(
    ...
    """Get offerings filtered by term and/or course."""
...
def _log_offering_program_enrichment(offerings: List[Dict[str, Any]]) -> None:
    """Log information about program enrichment results."""
    ...
    logger.info(
        f"[/api/offerings] After enrichment: {len(offerings_with_programs)} "
        f"with programs, {len(offerings_without_programs)} without programs"
    )
    if offerings_without_programs:
        logger.warning(
            f"[/api/offerings] Offerings without programs: "
            f"{[o.get('course_name', 'Unknown') for o in offerings_without_programs]}"
        )
...
def create_course_offering_endpoint() -> ResponseReturnValue:
    ...
    - sections: List of section data (optional, creates default if not provided)
    """
    try:
        ...
        offering_id = create_course_offering(offering_payload)
        ...
        if offering_id:
            ...
            for sec in sections_data:
                ...
                create_course_section(section_payload)
...
def update_course_offering_endpoint(offering_id: str) -> ResponseReturnValue:
    ...
    try:
        ...
        # Remove status fields that should not be directly updated
        for field in (
            "status",
            "term_status",
            "timeline_status",
