from unittest.mock import ANY, MagicMock, patch  # ANY helps match timestamp
...
def test_save_course_success(mocker):
    ...
    mock_collection.add.return_value = (
        None,
        mock_doc_ref,
    )  # Firestore add returns (timestamp, doc_ref)

    # Patch the db instance within the database_service module
    mocker.patch("database_service.db", mock_db)
    mocker.patch(
        "database_service.firestore.SERVER_TIMESTAMP", "mock_timestamp"
...
def test_get_all_courses_success_empty(mocker):
    ...
    mock_collection.order_by.assert_called_once_with(
        "timestamp", direction=mocker.ANY
